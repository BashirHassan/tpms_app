...
const schemas = {
  create: z.object({
    body: z.object({
      ...
      max_gps_accuracy_meters: z.number().int().min(1).optional().nullable(),
      geofence_radius_m: z.number().int().min(50).max(5000).optional(),
      supervisor_gps_accuracy_m: z.number().int().min(10).max(1000).optional(),
...
const create = async (req, res, next) => {
  try {
    ...
    const fields = [
      ...
      'max_supervision_visits', 'max_gps_accuracy_meters', 'geofence_radius_m', 'supervisor_gps_accuracy_m',
      'scoring_type', 'max_students_per_merged_group',
    ...
    const values = [
      ...
      data.max_gps_accuracy_meters != null ? parseInt(data.max_gps_accuracy_meters) : null,
      data.geofence_radius_m || 100,
      data.supervisor_gps_accuracy_m || 100,
...
const update = async (req, res, next) => {
  try {
    ...
    const allowedFields = [
      ...
      'inside_distance_threshold_km', 'max_supervision_visits', 'max_gps_accuracy_meters',
      'geofence_radius_m', 'supervisor_gps_accuracy_m', 'scoring_type',
