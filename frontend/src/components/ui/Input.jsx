...
const Input = forwardRef(({ className, type = 'text', label, error, helperText, id, disablePasswordToggle = false, onChange, value, onFocus, onBlur, ...props }, ref) => {
  ...
  return (
    <div className="w-full">
      {label && (
        <label htmlFor={inputId} className="flex text-sm font-medium text-gray-700 mb-1">
          {label}
      ...
      <div className={cn('relative', isPassword && 'flex items-center')}>
        <input
          ...
          type={resolvedType}
          className={cn(
            'flex h-12 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:cursor-not-allowed disabled:opacity-50',
